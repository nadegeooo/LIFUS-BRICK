...
def find_checkpoint(batch_size: int) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError(f"No checkpoint found for batch_size={batch_size} matching results/training/{pattern}")
    if len(candidates) > 1:
...
def analyze_checkpoint(checkpoint_path: Path, subjects: list) -> dict:
    """
    Returns {target: (roi_df, consistency_df)}. subjects is passed in
    (loaded once by the caller) rather than reloaded here, since the raw
    data doesn't change across checkpoints.
    """
...
def ensure_analyzed(batch_size: int, subjects: list, force: bool = False) -> Path:
    ...
    print(f"batch_size={batch_size}: no cached results at {cdir}, analyzing {checkpoint_path} ...")
...
def load_batch_size_data(force: bool = False) -> pd.DataFrame:
    ...
    for bs in BATCH_SIZES:
        cdir = ensure_analyzed(bs, subjects, force=force)
        for target in TARGETS:
            ...
            for _, r in roi_df.iterrows():
                ...
                rows.append({
                    "target": target, "roi": roi, "batch_size": bs,
                    "delta": r["delta"], "p_value_fdr": r["p_value_fdr"],
...
def plot_target(df: pd.DataFrame, target: str, ylim: tuple):
    ...
    fig.suptitle(f"\u0394C by batch size, per ROI \u2014 {target.upper()}", fontsize=13, y=1.05)
    fig.tight_layout()
